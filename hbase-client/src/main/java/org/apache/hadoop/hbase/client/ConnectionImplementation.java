...
class ConnectionImplementation implements ClusterConnection, Closeable {
    ...
    private final class MasterServiceStubMaker {
        ...
        private MasterProtos.MasterService.BlockingInterface makeStubNoRetries()
            ...
            /**
             * 在这里解析master地址(hg.master,16000,1558360619358)，将其中的端口号换成映射后的端口号
             */
            String actualMaster = ReadConf.getProperty("ACTUAL_MASTER");
            String expectMaster = ReadConf.getProperty("EXPECT_MASTER");
            if (sn.getServerName().startsWith(actualMaster)) {
                String hostname = expectMaster.split(",")[0];
                int port = Integer.parseInt(expectMaster.split(",")[1]);
                sn = ServerName.valueOf(hostname,port,sn.getStartcode());
            }
            if (isDeadServer(sn)) {
            ...
            ServerName finalSn = sn;
            MasterProtos.MasterService.BlockingInterface stub =
                    (MasterProtos.MasterService.BlockingInterface) computeIfAbsentEx(stubs, key, () -> {
                        BlockingRpcChannel channel = rpcClient.createBlockingRpcChannel(finalSn, user, rpcTimeout);
